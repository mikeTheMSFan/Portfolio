...
public class DeletePersonalDataModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (!string.IsNullOrEmpty(user.FileName))
        {
            ...
            if (match.Success)
            {
                var fileExtension = user.FileName.Substring(user.FileName.Length >= 4 ? user.FileName.Length - 4 : 0);
                var fileName = $"{match.Value}{fileExtension}";
                _remoteImageService.DeleteRemoteFile(uploadDirectory, fileName);
            }
        }
